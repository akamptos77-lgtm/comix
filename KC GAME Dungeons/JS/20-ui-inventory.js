'use strict';
/* 20-UI-INVENTORY: инвентарь, эликсиры, сравнение предметов */
...
function compareTxt(it){
  ...
  var cur = null;
  if(it.slot === 'ring'){
    cur = h.equip.ring1 || h.equip.ring2 || null;
  } else {
    cur = h.equip[it.slot];
  }
...
function renderElixirs(){
  ...
  el.innerHTML = h.elixirs.map(function(et, i){
    return '<button class="ebtn" data-el="' + i + '" data-tip="' +
      ELIXIRS[et].n + ': ' + ELIXIRS[et].d + '">' +
      '<span class="ei">' + ELIXIRS[et].i + '</span>' +
      ELIXIRS[et].n + '<br><small>' + ELIXIRS[et].d + '</small></button>';
  }).join('');
...
function useElixir(i){
  ...
  log('🧪 Брошен ' + ex.n + '!');
  var projType = et === 'freeze' ? 'icebolt' :
                 et === 'burn'   ? 'fireball' :
                 et === 'thunder' ? 'lightning' :
                 et === 'poison' ? 'poison' : 'arcane';
  fireProjectile(projType, 225, 320, 700, 320).then(function(){
    if(et === 'stun'){
      stunEnemy(2);
    } else if(et === 'freeze'){
      freezeEnemy(Math.round(getHeroAtk() * .8));
    } else if(et === 'burn'){
      burnEnemy(3, Math.max(3, Math.round(getHeroAtk() * .4)));
      return heroStrike(.5, {el:'fire', word:'ГОРИ!'});
    } else if(et === 'poison'){
      poisonEnemy(3, Math.max(3, Math.round(getHeroAtk() * .35)));
    } else if(et === 'heal'){
      healHero(.4);
    } else if(et === 'thunder'){
      return heroStrike(1.8, {el:'lightning', word:'ГРОМ!', big:true});
    }
...
function renderInv(){
  ...
  if(equipRow){
    equipRow.innerHTML = slots.map(function(sl){
      var it = h.equip[sl];
      var tip = it ?
        it.n + ': ' + bonusTxt(it) + (it.cursed ? ' ПРОКЛЯТО: ' + it.curse : '') + '. Клик чтобы снять.' :
        SLOT_NAME[sl] + ': пусто.';
      return '<div class="inv-slot ' + (it ? '' : 'empty') +
        (it && it.cursed ? ' cursed' : '') +
        '" data-slot="' + sl + '" data-tip="' + tip + '">' +
        '<small>' + SLOT_NAME[sl] + '</small>' +
        (it ?
          '<div class="ii">' + it.i + '</div><b>' + it.n +
          (it.up ? ' <span class="up-tag">+' + it.up + '</span>' : '') +
          '</b><span>' + bonusTxt(it) + '</span>' +
  ...
  if(bagRow){
    bagRow.innerHTML = h.inv.length ?
      h.inv.map(function(it, i){
        ...
        var tip = it.cursed ?
          it.n + ': ' + bonusTxt(it) + '. ПРОКЛЯТИЕ: ' + it.curse + '. ' + cmp :
          it.n + ': ' + bonusTxt(it) + '. ' + SLOT_NAME[it.slot] + '. ' + cmp;
        return '<div class="bag-item rar' + it.rar + (it.cursed ? ' cursed' : '') + '" data-tip="' + tip + '">' +
          ...
          '<small>' + SLOT_NAME[it.slot] +
          (!suitable ? ' ⚠ не твой класс' : '') +
          (it.cursed ? ' ☠ ' + it.curse : '') + '</small>' +
  ...
  var matsHtml = mats.length ?
    ...
    mats.map(function(m){
      return '<div class="bag-item" data-tip="Ингредиент для крафта ⚒️">' +
        '<b>🔮 ' + m + '</b><span>×' + G.materials[m] + '</span></div>';
    }).join('') + '</div>' : '';
