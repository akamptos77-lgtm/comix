...
function dropItem(minRar){
  ...
  pool.forEach(function(it){
    ...
    for(var i = 0; i < w; i++) weighted.push(it);
  });
...
/* =============================================
   КВЕСТЫ: прогресс → очередь → модалка выбора
   (фикс: было q.reward, стало q.rewards)
   ============================================= */
...
function renderShop(){
  ...
  el.innerHTML = '<div class="ev"><h3 class="ev-title">🛒 ЛАВКА</h3>' +
    '<div class="ev-anim">🧙</div><p>Золото: <b>' + G.gold + '</b> 💰 <small>(цены растут с этажом)</small></p>' +
    '<div class="ev-shop">' +
    G.shopGoods.map(function(s, i){
      var price = Math.round(s.p * mult);
      return '<button class="shop-it" data-i="' + i + '" ' + (G.gold < price ? 'disabled' : '') + '>' +
        '<span class="si-i">' + s.i + '</span><b>' + s.n + '</b>' +
        '<small>' + s.d + '</small><span class="p">' + price + ' 💰</span></button>';
    }).join('') + '</div>' +
    '<button class="cbtn ghost" id="shop-exit" style="margin-top:10px">Уйти →</button></div>';
...
function generateShopGoods(){
  ...
  goods.push({kind:'consume', i:'🍖', n:'Похлёбка', d:'Лечит 50% HP', p:40,
    b:function(h){ h.hp = Math.min(pMaxHp(), h.hp + Math.round(pMaxHp() * .5)); }});
