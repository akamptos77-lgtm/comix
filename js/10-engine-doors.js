'use strict';
/* ============================================
   10-ENGINE-DOORS: двери БЕЗ квестов,
   секретные комнаты, аркада, автосохранение
   ============================================ */

var DOOR_RESULT_LABEL={
  fight:'⚔️ Бой!',elite:'⭐ Элита!',boss:'💀 БОСС!',final:'☠ ФИНАЛ!',
  chest:'🎁 Сундук',shop:'🛒 Лавка',rest:'🔥 Привал',riddle:'🧩 Загадка',
...
function getDoorRevealIcon(t){
  var m={fight:'⚔️',elite:'⭐',boss:'💀',final:'☠',chest:'🎁',shop:'🛒',rest:'🔥',
  riddle:'🧩',fount:'⛲',shrine:'🕯️',trap:'🕸️',tavern:'🍺',library:'📚',skill:'📖',
  companion:'🆘',cursed:'💰',dummy:'🥊',secret:'🗝️',arcade:'🎰'};
  return m[t]||'🚪';
}

/* --- Генерация дверей (КВЕСТОВ НЕТ) --- */
function makeDoors(){
  ...
  var w={
    ...
    companion:.06, cursed:.03, dummy:.03, arcade:.05
  };
  for(var t in w){
...
function renderDoors(){
  ...
  var bfx=getBiomeFx();
  el.innerHTML='<div class="ev"><h3 class="ev-title">'+bio.ico+' ЭТАЖ '+G.floor+(isBossFloor?' — БОСС!':'')+'</h3>'+
    ...
    '<div class="doors">'+doors.map(function(d,i){
      return'<button class="door '+(d.type==='boss'||d.type==='final'?'boss':'')+(d.revealed?' opened':'')+(d.revealed==='selected'?' selected':'')+'" data-i="'+i+'" '+(d.revealed?'disabled':'')+'>'+
      '<div class="door-ico">'+(d.revealed?d.revealed:d.ico)+'</div>'+
      '<div class="door-hint">'+(d.revealed?(DOOR_RESULT_LABEL[d.type]||''):d.hint)+'</div></button>';
    }).join('')+'</div></div>';
  el.querySelectorAll('.door').forEach(function(b){
    if(b.disabled)return;
    b.onclick=function(){sfx.door();openDoor(parseInt(this.dataset.i,10));};
  });
}

/* --- Открытие двери (КВЕСТОВ НЕТ) --- */
function openDoor(i){
  var d=G.doors[i];
...
function afterEvent(){
  ...
  saveRun();
  var el=$('#event-layer');
  el.innerHTML='<div class="ev"><p><b>Этаж пройден!</b></p><button class="cbtn" id="btn-next" style="background:var(--yel)">Спуститься ниже ▼</button></div>';
...
function nextFloor(){
  ...
  updateHUD();
  var bio=getBiome(G.floor);
  log(bio.ico+' Этаж '+G.floor+' — '+bio.name+(G.floor%10===0?' — БОСС!':G.floor===100?' — ФИНАЛ!':''));
  var bfx=getBiomeFx();
