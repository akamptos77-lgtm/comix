'use strict';
/* ============================================
20-UI-INVENTORY: инвентарь, экипировка, эликсиры
ФИКС: эликсир = бросок (герой НЕ бежит и НЕ бьёт)
============================================ */
function itemLvl(it){return (it.tier||0)*3+(it.up||0);}
function lvlTag(it){var l=itemLvl(it);return l>0?' <span class="up-tag">ур.'+l+'</span>':'';}
...
function compareTxt(it){
  ...
  for(var k in keys){
    ...
    if(a||b){var d=Math.round((b-a)*100)/100;out.push(keys[k]+' '+(d>=0?'+':'')+d);}
  }
...
function renderElixirs(){
  ...
  el.innerHTML=h.elixirs.map(function(et,i){
    var ex=ELIXIRS[et];
    return'<button class="ebtn" data-el="'+i+'" data-tip="'+ex.n+': '+ex.d+'">'+
      '<span class="ei">'+ex.i+'</span>'+ex.n+'<br><small>'+ex.d+'</small></button>';
  }).join('');
...
/* === БРОСОК ЭЛИКСИРА: герой стоит и кидает, НЕ бежит/бьёт === */
function useElixir(i){
  ...
  var ex=ELIXIRS[et];
  log('🧪 Брошен '+ex.i+' '+ex.n+'!');
  enemyReact(['Что ты бросил?!','Это не честно!','Ай!']);

  var proj=et==='freeze'?'icebolt':et==='burn'?'fireball':et==='thunder'?'lightning':et==='poison'?'poison':'arcane';

  /* Только снаряд-бросок. Никакого рывка героя (h.fx.lunge не трогаем). */
  fireProjectile(proj,225,320,700,320).then(function(){
    var dmg;
    if(et==='stun'){stunEnemy(2);}
    else if(et==='freeze'){freezeEnemy(Math.round(getHeroAtk()*.8));}
    else if(et==='burn'){burnEnemy(3,Math.max(3,Math.round(getHeroAtk()*.4)));}
    else if(et==='poison'){poisonEnemy(3,Math.max(3,Math.round(getHeroAtk()*.35)));}
    else if(et==='heal'){healHero(.4);}
    else if(et==='thunder'){
      /* прямой урон молнией БЕЗ рывка героя */
      dmg=Math.round(getHeroAtk()*1.8*(1+Math.min(50,e.def||0)*0.01));
      return hitEnemy(dmg,{word:'ГРОМ!',big:true});
    }
...
function renderInv(){
  ...
  if(equipRow){
    equipRow.innerHTML=slots.map(function(sl){
      var it=h.equip[sl];
      var tip=it?it.n+': '+bonusTxt(it)+(it.cursed?'. ПРОКЛЯТО: '+it.curse:'')+'. Клик — снять.':SLOT_NAME[sl]+': пусто.';
      return'<div class="inv-slot '+(it?'':'empty')+(it&&it.cursed?' cursed':'')+'" data-slot="'+sl+'" data-tip="'+tip+'">'+
        '<small>'+SLOT_NAME[sl]+'</small>'+
        (it?'<div class="ii">'+it.i+'</div><b>'+it.n+lvlTag(it)+'</b><span>'+bonusTxt(it)+'</span>'+(it.cursed?'<span style="color:#8a1eff">☠ ПРОКЛЯТО</span>':'')+'<small style="color:#8a2222">снять ↩</small>'
  ...
  var matsHtml=mats.length?'<div class="bag-title">🔮 ИНГРЕДИЕНТЫ</div><div class="bag-row">'+
    mats.map(function(m){return'<div class="bag-item" data-tip="Ингредиент для крафта ⚒️"><b>🔮 '+m+'</b><span>×'+G.materials[m]+'</span></div>';}).join('')+'</div>':'';
  var invStats=$('#invStats');
  if(invStats){
    invStats.innerHTML='⚔️ Атака: <b>'+pAtk()+'</b> · 🛡️ Защита: <b>'+pDef()+'</b> · ❤️ HP: <b>'+pMaxHp()+'</b><br>'+
