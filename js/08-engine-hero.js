...
/* === КРИТ: кап 70% === */
function pCrit(){
  var c = G.hero.crit + G.hero.stats.agi * 3 + eqBonus('crit');
  if (cardsDone()) c += 5;
  return Math.min(70, Math.round(c * 10) / 10);
}

/* === УКЛОНЕНИЕ: кап 65% === */
function pDodge(){
  var d = G.hero.stats.agi * 2 + eqBonus('dodge') + G.hero.spd * 0.5;
...
/* === СИЛА НАВЫКОВ: кап ×2.5 === */
function pSkillPow(){
  var p = 1 + G.hero.stats.int * 0.1;
  return Math.min(2.5, p);
}

function pPotionPow(){return Math.min(1.0, 0.5 + G.hero.stats.int * 0.05);}
