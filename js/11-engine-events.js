...
/* === ПРИВАЛ: отдых, тренировка, кузница, целитель, реликвии === */
function openRest(){
  var el=$('#event-layer');
  ...
  $('#rest-doc').onclick=function(){
    ...
    G.gold-=cost;
    G.hero.hp=pMaxHp();
    G.hero.poison=null;G.hero.burn=null;
...
/* === КУЗНИЦА: крафт + улучшение === */
function upCost(it){return Math.round((40+it.rar*40)*((it.up||0)+1));}
...
function openSmith(tab){
  ...
  if(tab==='craft'){
    ...
    /* УЛУЧШЕНИЕ: экипировка + сумка, до +3 */
    var rows='';
...
function upRow(it,key,where){
  ...
  return'<div class="bag-item rar'+it.rar+'"><b>'+it.i+' '+it.n+(it.up?' <span class="up-tag">+'+it.up+'</span>':'')+' <span class="rar-tag">'+RAR[it.rar]+'</span></b>'+
    '<span>'+bonusTxt(it)+'</span><small>'+where+'</small>'+
    '<div class="bag-actions"><button class="cbtn small grn" data-up="'+key+'" '+((maxed||G.gold<c)?'disabled':'')+'>'+
    (maxed?'✨ МАКС':'🔨 +'+((it.up||0)+1)+' ('+c+'💰)')+'</button></div></div>';
}
