'use strict';
/* 11-ENGINE-EVENTS: события + ВЗЛОМ СУНДУКОВ */
...
function openRest(){var el=$('#event-layer');var mats=Object.keys(G.materials).filter(function(m){return G.materials[m]>0;});var hasMats=mats.length>0;
  el.innerHTML='<div class="ev"><h3 class="ev-title">🔥 ПРИВАЛ</h3><div class="ev-anim anim-glow">🏕️</div><p>Костёр потрескивает...</p><div class="ev-choices"><button class="cbtn grn" id="rest-heal">😴 Отдохнуть +35% HP</button><button class="cbtn red" id="rest-train">🏋️ Тренировка +2 атаки</button><button class="cbtn blu" id="rest-craft" '+(!hasMats?'disabled':'')+'>⚒️ Крафт ('+mats.length+' ингр.)</button></div></div>';
...
function openCraft(){
  var el=$('#event-layer'),mats=G.materials;
  var recipesHtml=RECIPES.map(function(rec,idx){
    var canCraft=true;
    var matsStr=Object.keys(rec.mats).map(function(m){var have=mats[m]||0,need=rec.mats[m];if(have<need)canCraft=false;return'<span style="color:'+(have>=need?'#2a8a4a':'#c44')+'">'+m+' '+have+'/'+need+'</span>';}).join(' · ');
  ...
  el.innerHTML='<div class="ev" style="max-width:720px"><h3 class="ev-title">⚒️ КУЗНИЦА</h3><p style="font-size:13px;opacity:.8;margin-bottom:10px">Собери ингредиенты и создай предмет!</p><div class="bag-row" style="max-height:300px;overflow-y:auto">'+recipesHtml+'</div><button class="cbtn ghost" id="craft-back" style="margin-top:14px">← Назад</button></div>';
...
function openFount(){var el=$('#event-layer');var fishCost=window._fishCost||5;
  el.innerHTML='<div class="ev"><h3 class="ev-title">⛲ ФОНТАН</h3><div class="ev-anim anim-glow">⛲</div><div class="ev-choices"><button class="cbtn grn" id="f-drink">💧 +30% HP</button><button class="cbtn" id="f-bottle" style="background:var(--yel)">🧪 +1 зелье</button><button class="cbtn red" id="f-dive">🏊 Нырнуть</button><button class="cbtn blu" id="f-fish" '+(G.gold<fishCost?'disabled':'')+'>🎣 Рыбалка ('+fishCost+'💰)</button></div></div>';
...
function openShrine(){var el=$('#event-layer');
  el.innerHTML='<div class="ev"><h3 class="ev-title">🕯️ АЛТАРЬ</h3><div class="ev-anim anim-glow">🕯️</div><div class="ev-choices"><button class="cbtn" id="sh-pray" style="background:var(--yel)">🙏 Молиться</button><button class="cbtn grn" id="sh-give" '+(G.gold<20?'disabled':'')+'>💰 20 золота</button><button class="cbtn ghost" id="sh-leave">Уйти</button></div></div>';
...
function openTrap(){var dodgeChance=Math.min(.85,.4+G.hero.stats.agi*.04);if(Math.random()<dodgeChance){sfx.click();log('🕸️ Уклонился!');}else{var dm=ri(8,14)+Math.floor(G.floor/2);G.hero.hp=Math.max(1,G.hero.hp-dm);sfx.hurt();log('🕸️ Ловушка! −'+dm+' HP');}updateHUD();afterEvent();}
function openTavern(){var el=$('#event-layer');
...
function openLibrary(){var cost=30+G.floor,el=$('#event-layer');
...
function openSkillEvent(){var el=$('#event-layer');
...
function showSkillLearned(s){var el=$('#event-layer');
...
function openCompanionEvent(){var el=$('#event-layer'),opts=[COMPANIONS.knight,COMPANIONS.wolf,COMPANIONS.fairy_c],comp=pick(opts);
...
function openDummy(){var el=$('#event-layer');
...
function openCursed(){var amt=ri(30,60)+G.floor*2,el=$('#event-layer');
