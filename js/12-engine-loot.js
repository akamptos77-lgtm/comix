...
function dropItem(minRar){
  ...
  if(Math.random()<.08){var cursed=pick(CURSED_ITEMS);var copy={};for(var k in cursed)copy[k]=cursed[k];copy.b={};for(var b in cursed.b)copy.b[b]=cursed.b[b];copy.up=0;copy.cursed=true;return copy;}
  ...
  var weighted=[];pool.forEach(function(it){var w=[5,2,1][it.rar]||1;for(var i=0;i<w;i++)weighted.push(it);});
  var src=pick(weighted);
  var copy2={slot:src.slot,i:src.i,n:src.n,rar:src.rar,b:{},up:0,cls:src.cls,el:src.el};
  for(var b2 in src.b)copy2.b[b2]=src.b[b2];return copy2;
}

function giveItem(it){
  if(G.hero.inv.length>=24){log('🎒 Сумка полна!');return false;}
  G.hero.inv.push(it);log('🎒 +'+it.i+' '+it.n+(it.cursed?' (ПРОКЛЯТО!)':''));saveRun();return true;
}
function giveElixir(et){
  var h=G.hero;if(h.elixirs.length>=h.elixirCap){log('Ячейки эликсиров полны!');return false;}
  h.elixirs.push(et);log('🧪 +'+ELIXIRS[et].i+' '+ELIXIRS[et].n);saveRun();return true;
}

/* === РЕЛИКВИИ (восстановлены) === */
...
function bonusTxt(it){return Object.keys(it.b).map(function(k){var mul=1+(it.up||0)*.25;var val=Math.round(it.b[k]*mul*100)/100;var names={atk:'⚔️',def:'🛡️',hp:'❤️',crit:'🎯',dodge:'💨',vamp:'🩸'};return'+'+val+(names[k]||k);}).join(' · ');}
function sellPrice(it){return Math.round(([8,18,40][it.rar]||8)*(1+(it.up||0)*.5));}
function getActiveSkill(){if(!G.hero||!G.hero.activeSkill)return null;var book=SKILL_BOOKS[G.hero.cls];for(var i=0;i<book.length;i++)if(book[i].id===G.hero.activeSkill)return book[i];return null;}
function grantRandomSkill(){
  ...
  if(!h.activeSkill)h.activeSkill=s.id;sfx.mystic();saveRun();return s;
}

/* === КВЕСТЫ (выбор награды) === */
...
function showQuestComplete(q){
  ...
  el.querySelectorAll('[data-reward]').forEach(function(b){
    b.onclick=function(){
      ...
      G.gold+=reward.gold;
      if(reward.item){
        var item={slot:reward.item.slot,i:reward.item.i,n:reward.item.n,rar:reward.item.rar,b:{},up:0};
        for(var k in reward.item.b)item.b[k]=reward.item.b[k];
        giveItem(item);
      }
      sfx.win();log('🎉 Награда получена!');updateHUD();
      G.pendingQuests.shift();
      if(G.pendingQuests.length)showQuestComplete(G.pendingQuests[0]);
      else closeOvl('ovl-quest');
      saveRun();
    };
  });
}
...
function renderShop(){
  ...
  el.innerHTML='<div class="ev"><h3 class="ev-title">🛒 ЛАВКА</h3><div class="ev-anim">🧙</div><p>Золото: <b>'+G.gold+'</b> 💰 <small>(цены растут с этажом)</small></p><div class="ev-shop">'+
    G.shopGoods.map(function(s,i){var price=Math.round(s.p*mult);return'<button class="shop-it" data-i="'+i+'" '+(G.gold<price?'disabled':'')+'><span class="si-i">'+s.i+'</span><b>'+s.n+'</b><small>'+s.d+'</small><span class="p">'+price+' 💰</span></button>';}).join('')+
    '</div><button class="cbtn ghost" id="shop-exit" style="margin-top:10px">Уйти →</button></div>';
  el.querySelectorAll('.shop-it').forEach(function(b){
    b.onclick=function(){
      ...
      else if(item.kind==='elixir'){giveElixir(item.et);log('Куплен: '+ELIXIRS[item.et].n);}
      else{item.b(G.hero);log('Куплено: '+item.n);}
      G.shopGoods.splice(parseInt(this.dataset.i,10),1);
      updateHUD();saveRun();renderShop();
    };
  });
