...
/* 11b-ENGINE-ARCADE: игровой зал + рыбалка.
   ...
   ИСПРАВЛЕНО: синтаксическая ошибка в startCountdown */

/* --- Пояснение перед игрой --- */
...
if(fill){
var p=left/seconds*100;
fill.style.width=p+'%';
fill.style.background=p>50?'var(--grn)':(p>25?'var(--yel)':'var(--red)');
}
},100);
...
/* === 💪 АРМРЕСТЛИНГ (таймер 7с) === */
function armWrestle(){gameIntro('💪 АРМРЕСТЛИНГ','💪','Быстро жми <b>«ЖМИ!»</b>, чтобы зелёная сторона выдавила красную. У тебя <b>7 секунд</b>.',armWrestleGame);}
function armWrestleGame(){
var bet=10;if(G.gold<bet){log('Не хватает золота!');return;}
G.gold-=bet;updateHUD();
var el=$('#event-layer'),pos=50,done=false,cancelTimer=null;
var rate=Math.min(1.6,0.7+G.floor*0.015);
el.innerHTML='<div class="ev"><h3 class="ev-title">💪 АРМРЕСТЛИНГ</h3><p>Жми! Твоя <b style="color:#2a8a4a">зелёная</b> линия борется с <b style="color:#c44">красной</b>.</p>'+timerBarHtml()+
...
function draw(){
var y=$('#arm-you'),f=$('#arm-foe'),p=$('#arm-pin');
...
cancelTimer=startCountdown(7,function(){if(!done)end(pos>=50,'Время вышло!');});
...
G.gold+=bet*2;sfx.gold();updateHUD();
...
btn.style.left=ri(5,80)+'%';btn.style.top=ri(5,70)+'%';
});
},800);
cancelTimer=startCountdown(8,function(){
if(!done){done=true;clearInterval(moveIv);sfx.hurt();showArcadeResult('🎯 ТИР','⏱️','Время вышло! Попадания: '+hits+'/'+targets);}
});
}
...
cancelTimer=startCountdown(12,function(){
if(done)return;done=true;
clearInterval(spawnIv);
var win=score*2;G.gold+=win;sfx.gold();updateHUD();
...
prizes[winIdx].f();
sfx.gold();updateHUD();
setTimeout(function(){showArcadeResult('🎡 КОЛЕСО ФОРТУНЫ',prizes[winIdx].i,'Выпало: '+prizes[winIdx].n);},700);
}
},100);
}
