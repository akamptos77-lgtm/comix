'use strict';
/* 09-ENGINE-COMBAT: бой. РЕФОРМА: заклинания не уклоняются,
...
var isSpell=(el!=='physical'); /* заклинание = стихия */
...
/* Уклониться можно ТОЛЬКО от физической атаки */
...
/* МОЛНИЯ: +1% урона за каждое очко защиты врага (до +50%) */
...
var mutation=null;
if(cycle>0&&Math.random()<0.3+cycle*0.1)mutation=pick(['regen','thorns','vampiric','enraged','shielded']);
var mutName=mutation?({regen:'♻️',thorns:'🌵',vampiric:'🩸',enraged:'💢',shielded:'🛡️'}[mutation]||'')+' ':'';
var e={id:base.id,name:mutName+base.name+(cycle>0?' +'+cycle:''),boss:boss,elite:elite,final:kind==='final',
tr:base.tr||null,ai:base.ai||'basic',el:base.el||'physical',weak:base.weak||null,resist:base.resist||null,
hp:Math.round(base.hp*hpM),maxHp:Math.round(base.hp*hpM),atk:Math.round(base.atk*atkM),def:Math.round(base.def*defM),
...
raged:false,stun:0,poison:null,burn:null,frozen:0,defending:false,nextAction:null,dead:false,mutation:mutation,fx:{lunge:0,hurt:0,death:1,enter:1}};
...
/* От ЗАКЛИНАНИЯ врага нельзя уклониться — только защититься */
if(!opts.magic&&Math.random()*100<getHeroDodge()){addFloat(225,200,'ВЖУХ!','#9fd8ff');log('Уклонение!');
if(hasRelic('ghoststep')){var gh=Math.max(1,Math.round(pMaxHp()*.03));h.hp=Math.min(pMaxHp(),h.hp+gh);addFloat(225,210,'👻+'+gh,'#7ef29a',18);}
...
var h=G.hero;
if(h.poison&&h.poison.turns>0){h.hp=Math.max(0,h.hp-h.poison.dmg);h.poison.turns--;addFloat(225,250,'☠−'+h.poison.dmg,'#b6ff5e',20);log('Яд: −'+h.poison.dmg);updateHUD();if(h.poison.turns<=0)h.poison=null;if(h.hp<=0){defeat();return;}sleep(400).then(function(){continueAction(a);});return;}
continueAction(a);
}
function continueAction(a){
var h=G.hero,e=G.enemy;G.busy=true;updateActions();
var buffBefore={};for(var bk in h.buffs)buffBefore[bk]=h.buffs[bk];
var chain=Promise.resolve();
...
for(var bk2 in h.buffs){if(h.buffs[bk2]>0&&(buffBefore[bk2]||0)>0)h.buffs[bk2]--;}
...
G.round++;
var bfx=getBiomeFx();
if(bfx&&bfx.tick&&G.round%3===0){var tick=Math.max(1,Math.round(pMaxHp()*.03));h.hp=Math.max(1,h.hp-tick);if(G.enemy&&!G.enemy.dead)G.enemy.hp=Math.max(1,G.enemy.hp-tick);addFloat(225,250,'☁−'+tick,'#b6e35e',18);log('🐸 Испарения: −'+tick+' HP обоим');updateHUD();}
saveRun();
}
function companionAttack(){
var c=G.companion,e=G.enemy;if(!c||!e||e.dead)return Promise.resolve();
log(c.icon+' '+c.name+' атакует!');
return fireProjectile('arcane',180,300,700,300).then(function(){var dmg=Math.max(1,Math.round((c.atk||10)*rand(.9,1.2)-e.def*.3));e.hp=Math.max(0,e.hp-dmg);addFloat(700,250,'−'+dmg,'#ffd8a8');addParts(700,240,'#ffd8a8',8);if(e.hp<=0)e.dead=true;updateHUD();return sleep(300);});
}
function winCombat(){
...
var goldBonus=Math.min(100,relicFxSum('goldPct'));if(goldBonus)g=Math.round(g*(1+goldBonus/100));
G.gold+=g;lines=['💰 Золото: +'+g,'✨ Опыт: +'+e.xp];
...
if(Math.random()<dropChance){var minR=e.boss?(e.final?2:1):0;var it=dropItem(minR);if(giveItem(it))lines.push(it.i+' '+(SLOT_NAME[it.slot]||'Предмет')+': '+it.n+(it.cursed?' (ПРОКЛЯТО!)':'')+' · '+bonusTxt(it));}
if((e.elite||e.boss)&&Math.random()<.25){var sk=grantRandomSkill();if(sk)lines.push(sk.icon+' Навык «'+sk.name+'»: '+sk.desc);}
if(e.boss&&Math.random()<.35){var rel=dropRelic();if(rel){giveRelic(rel);}else{G.gold+=100;lines.push('🏺 Все реликвии собраны! +100💰');}}
if(Math.random()<.3){var ek=pick(Object.keys(ELIXIRS));if(giveElixir(ek))lines.push(ELIXIRS[ek].i+' Эликсир: '+ELIXIRS[ek].n+' — '+ELIXIRS[ek].d);}
...
var el=$('#event-layer');
if(e.final){G.won=true;G.winBonus=true;
...
if(e.boss&&G.floor%10===0){el.innerHTML='<div class="ev"><h3 class="ev-title">👑 БОСС ПОВЕРЖЕН!</h3><div class="loot">'+lines.map(function(l){return'<div>'+l+'</div>';}).join('')+'</div><button class="cbtn" id="btn-next" style="background:var(--yel)">Спуститься ниже ▼</button></div>';$('#btn-next').onclick=function(){sfx.click();nextFloor();};flushQuests();return;}
...
if(h.hp<=0&&hasRelic('phoenix')&&phoenixReady()){h.hp=1;h.dead=false;G.phoenixCd=10;addFloat(225,200,'🪶 ФЕНИКС!','#ffd23d',24);log('🪶 Перо Феникса спасает героя!');sfx.level();updateHUD();return Promise.resolve();}
