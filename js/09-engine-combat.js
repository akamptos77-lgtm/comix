...
function heroStrike(mult, opts){
  ...
  chain = chain.then(function(){
    ...
    if(!opts.sure && !isSpell && e.dodge && Math.random() < e.dodge){
...
function makeEnemy(base, kind, isMimic){
  ...
  /* Элитные враги на высоких этажах */
...
function doEnemyAttack(e, h, mult, opts){
  /* От заклинания (magic) НЕ уклоняемся — только защищаемся */
  if(!opts.magic && Math.random() * 100 < getHeroDodge()){
...
function continueAction(a){
  ...
  chain.then(function(){
    ...
    if(e && e.dead) return;
    if(h.hp <= 0) return defeat();
    if(supportTurn) return; /* ПОДДЕРЖКА: враг не отвечает */
    return enemyTurn();
  }).then(function(){
    if(G.phase !== 'combat') return;
    if(h.hp <= 0) return defeat();
    if(supportTurn){
      G.busy = false; updateHUD(); updateActions(); saveRun();
      log('✨ Поддержка! Враг не успел ответить — можешь действовать.');
      return;
    }
    /* Бафы тикают ТОЛЬКО те, что были до хода героя */
    for(var bk2 in h.buffs){
      if(h.buffs[bk2] > 0 && (buffBefore[bk2] || 0) > 0) h.buffs[bk2]--;
    }
    roundEnd();
    h.defending = false;
    G.busy = false; updateHUD(); updateActions();
  });
}
...
function winCombat(){
  ...
  return sleep(800).then(function(){
    ...
    var monster = null;
    for(var i = 0; i < ALL_MONSTERS.length; i++) if(ALL_MONSTERS[i].id === e.id){ monster = ALL_MONSTERS[i]; break; }
    ...
    if(Math.random() < dropChance){
      ...
      if(giveItem(it)) lines.push(it.i + ' ' + (SLOT_NAME[it.slot] || 'Предмет') + ': ' + it.n + (it.cursed ? ' (ПРОКЛЯТО!)' : '') + ' · ' + bonusTxt(it));
    }
