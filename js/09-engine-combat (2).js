...
function makeEnemy(base,kind,isMimic){
  var dm=DIFF[G.diff],f=G.floor,cycle=G.cycle||0,cycleMult=1+cycle*0.5;
  ...
  if(isMimic){return{id:'mimic',name:'Мимик',boss:false,elite:false,tr:null,ai:'melee',el:'physical',weak:'fire',resist:null,hp:Math.round(60*hpM),maxHp:Math.round(60*hpM),atk:Math.round(20*atkM),def:Math.round(5*(1+.03*(f-1))*cycleMult),spd:8,crit:8,gold:Math.round(40*(1+.05*f)*cycleMult),xp:Math.round(50*(1+.1*f)*cycleMult),scale:1,dodge:.1,raged:false,stun:0,poison:null,burn:null,frozen:0,defending:false,nextAction:null,dead:false,fx:{lunge:0,hurt:0,death:1,enter:1}};}
  var e={id:base.id,name:base.name+(cycle>0?' +'+cycle:''),boss:boss,elite:elite,final:kind==='final',tr:base.tr||null,ai:base.ai||'basic',el:base.el||'physical',weak:base.weak||null,resist:base.resist||null,hp:Math.round(base.hp*hpM),maxHp:Math.round(base.hp*hpM),atk:Math.round(base.atk*atkM),def:Math.round(base.def*(1+.03*(f-1))*(elite?1.1:1)*cycleMult),spd:base.spd,crit:base.crit||5,
  gold:Math.round(base.gold*(1+.05*(f-1))*dm.gold*(elite?1.8:1)*(boss?1.5:1)*cycleMult),
  xp:Math.round(base.xp*(1+.1*(f-1))*(elite?1.8:1)*(boss?1.6:1)*cycleMult),scale:boss?1.28:elite?1.12:1,dodge:base.tr==='swift'?.22:0,raged:false,stun:0,poison:null,burn:null,frozen:0,defending:false,nextAction:null,dead:false,fx:{lunge:0,hurt:0,death:1,enter:1}};
...
function winCombat(){
  ...
  return sleep(800).then(function(){
    ...
    /* === МЕНЬШЕ ЗОЛОТА: срез 0.75 → 0.5 === */
    var g=Math.round(e.gold*0.5)+ri(-2,3);
    G.gold+=g;lines=['💰 +'+g+' золота','✨ +'+e.xp+' опыта'];
    ...
    if(e.boss&&Math.random()<.35){var rel=dropRelic();if(rel&&giveRelic(rel))lines.push(rel.i+' '+rel.n+'!');}
    ...
    if(e.final){
      ...
      el.innerHTML='<div class="ev"><h3 class="ev-title">🏆 ФИНАЛЬНАЯ ПОБЕДА!</h3><div class="ev-anim anim-glow">🏆</div><p>Пожиратель Миров повержен!</p><p style="font-size:14px;opacity:.8;margin:8px 0">Пройдено циклов: <b>'+(G.cycle||0)+'</b></p><div class="center"><button class="cbtn grn" id="win-continue">